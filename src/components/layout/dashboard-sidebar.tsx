...
export default function DashboardSidebar() {
  ...
  const links = (isSchool ? SCHOOL_LINKS : TEACHER_LINKS).filter((link) => {
    if (link.href === "/dashboard/messages") return featureFlags.messaging;
    if (link.href === "/dashboard/notifications") return featureFlags.notificationsCenter;
    if (link.href === "/dashboard/settings") return featureFlags.settingsSecurity;
    if (link.href === "/dashboard/pipeline") return featureFlags.pipelineBoard;
    if (link.href === "/dashboard/analytics") return isSchool;
    return true;
  });
      ...
      <div className="px-5 pt-5 pb-4 border-b border-black/[0.05]">
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 bg-brand-gradient rounded-lg flex items-center justify-center flex-shrink-0">
            {isSchool ? <Building2 size={12} className="text-white" /> : <GraduationCap size={12} className="text-white" />}
          </div>
          <div>
            <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-widest leading-none">Dashboard</p>
            <p className="text-xs font-medium text-gray-600 mt-0.5">{isSchool ? "School Admin" : "Teacher"}</p>
          </div>
        </div>
      </div>
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-medium transition-all duration-[120ms] mb-0.5 ${
                isActive ? "bg-brand-50 text-brand-700" : "text-gray-600 hover:bg-black/[0.04] hover:text-gray-900"
              }`}
            >
